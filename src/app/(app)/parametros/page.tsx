...
type SaleChannel = { id: number; name: string; is_active: boolean; sort_order: number; has_delivery: boolean; immediate_delivery: boolean; auto_invoice: boolean; afip_pos_id?: number | null; afip_pos_name?: string | null };
type AfipPoint = { id: number; punto_venta: number; name: string; production: boolean; is_default: boolean; has_certs: boolean };
...
function SaleChannelsABM() {
  ...
  const [form, setForm] = useState<{ name: string; sort_order: number; has_delivery?: boolean; immediate_delivery?: boolean; auto_invoice?: boolean; afip_pos_id?: string }>({ name: "", sort_order: 0, has_delivery: false, immediate_delivery: false, auto_invoice: false, afip_pos_id: "" });
  const [saving, setSaving] = useState(false);
  const [afipPoints, setAfipPoints] = useState<AfipPoint[]>([]);

  function load() {
    setLoading(true);
    Promise.all([fetchJson<SaleChannel[]>("/sale-channels"), fetchJson<AfipPoint[]>("/afip/points-config").catch(() => [])]).then(([channels, points]) => { setItems(channels); setAfipPoints(points); }).catch(console.error).finally(() => setLoading(false));
  }
  useEffect(() => { load(); }, []);
  function openNew() { setEditing(null); setForm({ name: "", sort_order: items.length + 1, has_delivery: false, immediate_delivery: false, auto_invoice: false, afip_pos_id: "" }); setShowForm(true); }
  function openEdit(c: SaleChannel) { setEditing(c); setForm({ name: c.name, sort_order: c.sort_order || 0, has_delivery: !!c.has_delivery, immediate_delivery: !!c.immediate_delivery, auto_invoice: !!c.auto_invoice, afip_pos_id: c.afip_pos_id ? String(c.afip_pos_id) : "" }); setShowForm(true); }
  async function handleSave() {
    if (!form.name.trim()) return;
    setSaving(true);
    try {
      const payload = { ...form, afip_pos_id: form.afip_pos_id ? Number(form.afip_pos_id) : null };
      if (editing) await putJson("/sale-channels/" + editing.id, payload);
      else await postJson("/sale-channels", payload);
      setShowForm(false); load();
    } catch (e) { console.error(e); } finally { setSaving(false); }
  }
  ...
  function renderItem(c: SaleChannel) {
    return (
      <div key={c.id} style={{ padding: "5px 0", borderBottom: "1px solid #f5", fontSize: "12px", display: "flex", alignItems: "center", gap: "6px" }}>
        ...
        {c.auto_invoice && <span title="Facturación automática" style={{ fontSize: "11px", color: "#8e44ad" }}>🧾</span>}
        {c.afip_pos_name && <span title="Punto de venta AFIP" style={{ fontSize: "10px", color: "#555", background: "#f0f0f0", borderRadius: "4px", padding: "1px 4px" }}>{c.afip_pos_name}</span>}
        <IconButton variant="ghost" title="Editar" onClick={() => openEdit(c)}>✏️</IconButton>
        <IconButton variant="danger" title="Eliminar" onClick={() => remove(c.id)}>🗑️</IconButton>
      </div>
    );
  ...
  return (
    <>
      ...
      <ModalForm show={showForm} onClose={() => setShowForm(false)} title={(editing ? "Editar" : "Nuevo") + " Canal de Venta"}>
              ...
              <label style={{ display: "flex", alignItems: "center", gap: "10px", fontSize: "13px", cursor: "pointer", padding: "10px", background: form.auto_invoice ? "#f4edff" : "#fafafa", border: "1px solid " + (form.auto_invoice ? "#b58cff" : "#eee"), borderRadius: "10px" }}>
                <input type="checkbox" checked={!!form.auto_invoice} onChange={e => setForm((prev) => ({ ...prev, auto_invoice: e.target.checked }))}
                  style={{ width: "18px", height: "18px", accentColor: "#8e44ad", cursor: "pointer" }} />
                <span style={{ fontWeight: 700 }}>🧾 Facturar automáticamente las ventas de este canal</span>
              </label>
              {form.auto_invoice && (
                <div style={{ fontSize: "11px", color: "#666", background: "#fbf8ff", border: "1px solid #eadcff", borderRadius: "8px", padding: "8px 10px" }}>
                  Al crear una NV con este canal, el sistema intentará emitir la factura electrónica en ARCA automáticamente. Si ARCA rechaza, la venta queda creada y el error queda informado.
                </div>
              )}
              <div>
                <label style={{ fontSize: "12px", fontWeight: 600, display: "block", marginBottom: "4px", color: "#555" }}>Punto de venta AFIP para este canal</label>
                <select value={form.afip_pos_id || ""} onChange={e => setForm(prev => ({ ...prev, afip_pos_id: e.target.value }))}
                  style={{ width: "100%", padding: "9px 10px", border: "1px solid #ddd", borderRadius: "8px", fontSize: "13px" }}>
                  <option value="">Usar punto de venta default</option>
                  {afipPoints.map(p => <option key={p.id} value={p.id}>{p.name || `PV ${p.punto_venta}`} · PV {p.punto_venta}{p.production ? " · Prod" : " · Homo"}{p.is_default ? " · Default" : ""}</option>)}
                </select>
              </div>
              <div style={{ display: "flex", gap: "10px", marginTop: "8px" }}>
                <button onClick={() => setShowForm(false)} style={{ flex: 1, padding: "10px", borderRadius: "10px", border: "2px solid #e0e0e0", background: "transparent", color: "#666", cursor: "pointer", fontWeight: 600, fontSize: "13px" }}>Cancelar</button>
